...
//		shadowtex = SDL_CreateTextureFromSurface(renderer, shadowsurf);
		...
		while(enter_pressed == false && quit == false)
		{
			if(SDL_PollEvent(&events))
			{
				...
				if(events.type == SDL_KEYDOWN && events.key.keysym.sym == SDLK_BACKSPACE && text.length() > 0)
				{
					text.pop_back();
					inputbox = TTF_RenderUTF8_Solid(font, text.c_str(), text_color);
					inputboxtex = SDL_CreateTextureFromSurface(renderer, inputbox);
...
//			}
		...
		if(who == 0)
		{
			...
			save_score(player_name.c_str(), player_time_secs, num_player_turns);
			quit = false;
			while(quit == false)
			{
				if(SDL_PollEvent(&events))
	...
	SDL_SetTextureAlphaMod(circletex, 255);
		...
		std::cout << "ALL LOOPS AFTER" << std::endl;
	while(finish == false && (circles_num + crosses_num) < 9 && playing_mode == 1)
	{
		...
		std::cout << "While AIloop()" << std::endl;

		if(playing_mode == 1)
		{
//			if(player_id == 1) /* Now 1 is human 0 is AI */
//			{
				std::cout << "AI_PLAY" << std::endl;
				SDL_RenderCopy(renderer, background, NULL, NULL);
...
//					SDL_RenderCopy(renderer, player1_turntex, NULL, &turnrect1);
				...
				if(who == 0)
				{
					...
					std::string str = "Gratulacje, wygrałeś/aś!, liczba ruchów: ";
					str += player_turns_str;
					str += "\n\nPodaj swoje imię i wciśnij ENTER aby wyjść z gry";
					...
					quit = false;
					SDL_Delay(1000);
					SDL_Surface *in = TTF_RenderUTF8_Solid(font, "_", text_color);
					SDL_Texture *tin = SDL_CreateTextureFromSurface(renderer, in);
					SDL_Rect t;
					t.w = in->w;
					t.h = in->h;
					t.x = (windowx / 2) - (t.w / 2);
					...
					while(quit == false)
					{
						if(SDL_PollEvent(&events))
						{
							...
							if(events.type == SDL_TEXTINPUT)
							{
								...
								in = TTF_RenderUTF8_Solid(font, name.c_str(), text_color);
								tin = SDL_CreateTextureFromSurface(renderer, in);
								t.w = in->w;
								t.h = in->h;
								t.x = (windowx / 2) - (t.w / 2);
								t.y = windowy / 2;
							...
							if(events.type == SDL_KEYDOWN && events.key.keysym.sym == SDLK_BACKSPACE && text.length() > 0)
							{
								...
								in = TTF_RenderUTF8_Solid(font, text.c_str(), text_color);
								tin = SDL_CreateTextureFromSurface(renderer, inputbox);
								t.w = in->w;
								t.h = in->h;
								t.x = (windowx / 2) - (t.w / 2);
								t.y = windowy / 2;
						...
						SDL_RenderCopy(renderer, background, NULL, NULL);
						SDL_RenderCopy(renderer, msgtex, NULL, &msgrect);
						SDL_RenderCopy(renderer, boxshadow, NULL, &t);
						SDL_RenderCopy(renderer, tin, NULL, &t);
						SDL_RenderPresent(renderer);
						/* Calculate time */
						player1_time_secs = player1_time_secs + (player1_turn_end.tv_sec - player1_turn_start.tv_sec) + ((player1_turn_end.tv_usec - player1_turn_start.tv_usec)/1000000.0);
					}
					...
					SDL_DestroyTexture(msgtex);
					msgtex = NULL;
					save_score(name.c_str(), player1_time_secs, num_player1_turns);
				}
				if(who == 1)
				{
					std::cout << "AI won!!!!" << std::endl;
					/* Render text */
					msgsurf = TTF_RenderUTF8_Blended_Wrapped(font, "Niestety przgrałeś/aś ale nie przejmuj się, następnym razem będzie lepiej.\nKliknij aby wyjść z gry", text_color, windowx - (windowx * 0.083));
					msgtex = SDL_CreateTextureFromSurface(renderer, msgsurf);
					msgrect.w = msgsurf->w;
					msgrect.h = msgsurf->h;
					msgrect.x = (windowx / 2) - (msgsurf->w / 2);
...
//				SDL_Delay(2000);
				...
				if(who == 0)
				{
					...
					char player_time_str[128];
					sprintf(player_time_str, "%'.3f", player1_time_secs);
					std::string str = "Gratulacje " + first_player + ", wygrałeś/aś!, liczba ruchów: ";
					str += player_turns_str;
					...
					SDL_DestroyTexture(msgtex);
					msgtex = NULL;
					save_score(first_player.c_str(), player1_time_secs, num_player1_turns);
				}
				if(who == 1)
				{
					...
					std::string str = "Gratulacje " + second_player + ", wygrałeś/aś!, liczba ruchów: ";
					str += player_turns_str;
					str += "\n\nKliknij aby wyjść z gry";
					msgsurf = TTF_RenderUTF8_Blended_Wrapped(font, str.c_str(), text_color, windowx - (windowx * 0.083));
					msgtex = SDL_CreateTextureFromSurface(renderer, msgsurf);
					...
					SDL_DestroyTexture(msgtex);
					msgtex = NULL;
					save_score(second_player.c_str(), player2_time_secs, num_player2_turns);
				}
				if(who == -1 && (circles_num + crosses_num) == 9)
				{
					/* No one won */
